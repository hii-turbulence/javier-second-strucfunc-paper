...
\begin{abstract}
  ...
  % What we found
  Masking and downsampling significantly improve the reliability and efficiency of structure function analysis. The fitted model successfully recovers the turbulent parameters for all binning levels, with higher bin sizes yielding better agreement with the "true" (noise-free, seeing-free) structure function until a limit where the binned pixels are order of magnitude X the correlation lenght. Differences in large-scale behavior among emission lines are observed, revealing line-specific turbulence signatures. Our method allows consistent recovery of turbulent parameters even under varying observational conditions. %Comparison with KPNO echelle yields...
  % Take-away statement
  We show that, despite its relatively poor velocity resolution, 
  our methodology can turn MUSE into a tool for studying turbulent velocity fluctuations down to \qty{1}{km.s^{-1}} at arececond scales.
\end{abstract}
...
The dotted lines corresponds to the heuristic model described in equation~\eqref{eq:sf-functional} and fitted using..., while the dashed lines represent the ideal model given in equation~\eqref{eq:model-strucfunc-ideal}.  
...
This is better seen in Table~\ref{tab:results_MUSE_Ha} where the value obtained for the instrumental noise, \( B_{\text{noise}} \), decrease from a value of \SI{\sim 40}{km.s^{-1}} to \SI{\sim 0.5}{km.s^{-1}}.
...
As a consequence, each binning level also leads to a decrease in the structure function values due to the reduced ability to sample velocity fluctuations at smaller spatial scales.  
This trend is reflected in Table~\ref{tab:results_MUSE_Ha}, where an increase in the upper limits of the fitted \(s_0\) values is observed for the \halpha\ emission line, ranging from \num{\sim 1} to \SI{\sim 6}{arcsec}.

%These values also serve as a sanity check to validate the consistency of the model fits.  
...
From Table~\ref{tab:results_MUSE_Ha} we see that there is a range for the binning procedure, in this case \num{2}-\num{8}, which allow to recover the turbulent cascade withing a noise value between \num{0.5} to \SI{1.5}{km^2 . s^2} and seeing below \SI{1.7}{arcsec} which is consistent with astronomical observations.
...
In this case the reduction in noise for the non-binned case to the binning of \num{16} is from \SI{10}{km.s^{-1}} to \SI{0.5}{km.s^{-1}}, while the upper limit detected for the seeing is \SI{\sim 6}{arcsec} for all binned maps. 

Despite the large upper limit found for the [ArIII] emission line, we observe a good overall agreement with the model described by equation~\eqref{eq:model-strucfunc-ideal}.  
This indicates that the turbulent cascade is consistently recovered across all maps, both binned and non-binned.
...
This demonstrates not only the applicability of the binning procedure in recovering the turbulent cascade in a computationally efficient manner, but also the effectiveness of the heuristic model for extracting the relevant turbulent parameters under certain S/N conditions.
...
This change is primarily driven by the difference in masking applied to the binned maps and non-binned map. 
This is significant, as it suggests that large-scale fluctuations may be line-dependent, which is consistent with the expected spatial distribution of different ions within the nebula.
...
\begin{figure*}
 ...
 \caption{
The solid markers shows the observational structure function, equation~\eqref{eq:Br}, for the non-binned map and binned maps. The dotted line corresponds to the heuristic model described in equation~\eqref{eq:model-strucfunc-ideal}, while the dashed lines represent the ideal model given in equation~\eqref{eq:sf-functional}.  Left panel: \halpha. Right panel: [ArIII]  
 }
\label{fig:figures/bin comparison}
\end{figure*}

\subsubsection{Other emission lines}\label{sec:Representative case study II}

In the Appendix~\ref{apex:multiple_lines_results} the Table~\ref{tab:results_MUSE_O}-\ref{tab:results_MUSE_S} shows the results for the different binning procedures for the [OIII], [NII] and [SIII] emission lines.
For each case the same procedure as the \halpha\ and [ArIII] emission is applied.
For the O lines the results are...

\subsection{Second-order structure function}

For our discussion of the turbulent parameters and for the data comparison (see next section), we choose to present the detailed results corresponding to a binning factor of \num{4}.  
The Figures~\ref{fig:Orion_sf_1}-\ref{fig:Orion_sf_S} shows the second-order structure functions for the all the emission lines studied in this work and their posterior distributions obtained bia MCMC (see Section~X).


\begin{figure*}
  \centering
  \sffigggg{H_I-6563_mask_bin_4}{H_I-6563_mask_bin_4}{Ar_III-7136_mask_bin_8}{Ar_III-7136_mask_bin_8}
  \caption{ 
   Left panel: Second-order structure functions
    for velocity centroid images
    of the \ha{}  (binning = 4) and [ArIII] emission lines (binning = 8).
    ...
    is shown in green.
    Right panel: Corner plot of covariances between
    model parameters of fits to the \ha{} and [ArIII] structure function.
...
\begin{figure*}
  \centering
  \sffigggg{OrionSO}{OrionSO}{OrionSN}{OrionSN}
  \caption{ 
  }
  \label{fig:Orion_sf_O}
\end{figure*}
